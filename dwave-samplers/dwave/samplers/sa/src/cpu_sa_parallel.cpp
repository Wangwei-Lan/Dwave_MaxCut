...
void simulated_annealing_run(
    ...
    bool   *whether_flip = (bool*)malloc(num_vars*sizeof(bool));
    uint64_t *rand_vector = (uint64_t*)malloc(num_vars*sizeof(uint64_t));
    uint64_t rand; // this will hold the value of the rng
    double universal_p = 1.0;
    ...
    double ediff_exp;
    double ediff;
    vector<double> inv_factor{1.0,1.0/2.0,1.0/6.0,1.0/24.0,1.0/120.0,1.0/720.0,1.0/5760.0}; 
    // perform the sweeps
    for (int beta_idx = 0; beta_idx < (int)beta_schedule.size(); beta_idx++) {
        ...
        for (int sweep = 0; sweep < sweeps_per_beta; sweep++) {
            ...
            // TODO: parallel update of all spins            
            // TODO: based on use OpenMP 

            FASTRAND(rand);

            #pragma omp parallel for
                for (int var = 0; var < num_vars; var++) {
                    // recalculate the local fields for all the spins 
                    // ! This maybe probamatic since in the serial cases, there is no need to calculate local field
                    // ! the calculation of local field is significantly slower than only update the local field 
                    for( int n_i = 0;n_i< degrees[var]; n_i++)
                    {
                        int neighbor = neighbors[var][n_i];
                        delta_energy[var] += neighbour_couplings[var][n_i] * state[neighbor];
                    }
                    delta_energy[var] += h[var];
                    delta_energy[var] *= state[var]; 
                    
                    FASTRAND(rand_vector[var]);

                    if (exp(-delta_energy[var]*beta)* RAND_MAX > rand_vector[var]) {
                        whether_flip[var] = true;
                    }
