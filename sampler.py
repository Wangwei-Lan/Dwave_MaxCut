...
if __name__ == "__main__":
    ...
    # (1) solve qubo model 
    instance_dict = formulate_maxcut_qubo_sparse(filename)
    running_time = []
    print("start calculations!")
    for numruns in [1,2,4,8,16,32,64]:
      start_time = time.time()
      sampleset = solver.sample_qubo(instance_dict,num_reads=numruns,beta_schedule_type='geometric',num_sweeps=1000,beta_range=[0.001,100000000.0],num_sweeps_per_beta=1,)
      end_time = time.time()
